...
public class MCCAnvil {
    ...
    public static void repairItem(@NotNull ItemStack firstItem, @NotNull ItemStack secondItem) {
        ...
        int repairCost = ContainerAnvil.e(Math.max(firstCraftItem.G(), secondCraftItem.G()));
    ...
    public static int getRepairCost(@NotNull ItemStack primaryItem, @NotNull ItemStack secondaryItem) {
        ...
        if (!craftPrimaryItem.g()) return 0;

        if (craftPrimaryItem.c().a(craftPrimaryItem, craftSecondaryItem)) {

            int repairCost = 0;

            int repairedDurability = Math.min(craftPrimaryItem.i(), craftPrimaryItem.j() / 4);
            if (repairedDurability <= 0) return 0;

            while (repairedDurability > 0 && repairCost < craftSecondaryItem.J()){

                int durability = craftPrimaryItem.i() - repairedDurability;
                craftPrimaryItem.b(durability);

                repairedDurability = Math.min(craftPrimaryItem.i(), craftPrimaryItem.j() / 4);
                repairCost++;
        ...
        if (primaryItem.getType() == secondaryItem.getType()) {
            ...
            int repairedDurability = craftPrimaryItem.j() - craftPrimaryItem.i();
            int repairCost = craftSecondaryItem.j() - craftSecondaryItem.i();

            int l = repairCost + craftPrimaryItem.j() * 12 / 100;
            int ji = repairedDurability + l;
            int k1 = craftPrimaryItem.j() - ji;

            if (k1 < 0) k1 = 0;
            if (k1 < craftPrimaryItem.i()) {
    ...
    public static int getUseCost(@NotNull ItemStack firstItem, @NotNull ItemStack secondItem) {
        ...
        int extraCost = firstCraftItem.G() + secondCraftItem.G();
        ...
        for (Map.Entry<Enchantment, Integer> entry : secondsItemsEnchantments.entrySet()) {
            ...
            if (firstCraftItem.J() > 1) repairCost = 40;
